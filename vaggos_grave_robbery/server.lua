data = {}
local VorpCore
local VorpInv
local QBRItems

if Config.framework == "redemrp" then
    TriggerEvent("redemrp_inventory:getData",function(call)
        data = call
    end)
elseif Config.framework == "vorp" then
    TriggerEvent("getCore",function(core)
        VorpCore = core
    end)
    
    VorpInv = exports.vorp_inventory:vorp_inventoryApi()
elseif Config.framework == "qbr" then 
    QBRItems = exports['qbr-core']:GetItems()
end

local TEXTS = Config.Texts
local TEXTURES = Config.Textures

local DiggedGraves = {}

RegisterServerEvent("vaggos_grave_robbery:check_shovel")
AddEventHandler("vaggos_grave_robbery:check_shovel", function(id)
    local _source = source
    if DiggedGraves[id] == true then 
        TriggerClientEvent("Notification:left_grave_robbery", _source, TEXTS.GraveRobbery, TEXTS.GraveRobbed, TEXTURES.alert[1], TEXTURES.alert[2], 2000)
        return 
    end
    local count = nil
    if Config.framework == "redemrp" then
        local itemD = data.getItem(_source, Config.ShovelItem)
        if itemD and itemD.ItemAmount > 0 then 
            count = itemD.ItemAmount
        end
    elseif Config.framework == "vorp" then
        count = VorpInv.getItemCount(_source, Config.ShovelItem)
    elseif Config.framework == "qbr" then 
        local User = exports['qbr-core']:GetPlayer(_source)
        local hasItem = User.Functions.GetItemByName(Config.ShovelItem)
        if hasItem and hasItem.amount > 0 then 
            count = hasItem.amount
        end
    end
    if count and count > 0 then
        TriggerClientEvent("vaggos_grave_robbery:start_dig", _source, id)
    else
        TriggerClientEvent("Notification:left_grave_robbery", _source, TEXTS.GraveRobbery, TEXTS.NoShovel, TEXTURES.alert[1], TEXTURES.alert[2], 2000)
    end
end)

RegisterServerEvent("vaggos_grave_robbery:reward")
AddEventHandler("vaggos_grave_robbery:reward", function(id)
    local _source = source
    Citizen.Wait(math.random(200,800))
    if DiggedGraves[id] == true then 
        TriggerClientEvent("Notification:left_grave_robbery", _source, TEXTS.GraveRobbery, TEXTS.GraveRobbed, TEXTURES.alert[1], TEXTURES.alert[2], 2000)
        return 
    end
    DiggedGraves[id] = true 
    local itemnr = math.random(1,#Config.Rewards)
    print(Config.Graves[id].items ~= nil)
    local temp_items = {} 
    if Config.Graves[id].items ~= nil then 
        for k, v in pairs(Config.Graves[id].items) do 
            local item = v.item
            local label = v.label
            local amount = v.amount
            local chance = v.chance
            local itemnr = math.random(1,100)
            print(itemnr)
            if chance >= itemnr then
                if Config.framework == "redemrp" then
                    local itemD = data.getItem(_source, item)
                    itemD.AddItem(amount)
                elseif Config.framework == "vorp" then
                    VorpInv.addItem(_source, item, amount)
                elseif Config.framework == "qbr" then
                    local User = exports['qbr-core']:GetPlayer(_source)
                    User.Functions.AddItem(item, amount)
                end
                item = {item = v.item, label = v.label, amount = v.amount}
                table.insert(temp_items, item)
            end
        end
        if #temp_items ~= 0 then
            for k,v in pairs(temp_items) do
                TriggerClientEvent("Notification:left_grave_robbery", _source, TEXTS.GraveRobbery, TEXTS.FoundItem.."\n+ ".. v.label, TEXTURES.alert[1], TEXTURES.alert[2], 2000)
                Wait(3000)
            end
        else
            TriggerClientEvent("Notification:left_grave_robbery", _source, TEXTS.GraveRobbery, TEXTS.FoundNoItem, TEXTURES.alert[1], TEXTURES.alert[2], 2000)
        end
    else
        local itemnr2 = math.random(1,100)
        if Config.Chance >= itemnr2 then
            local item = Config.Rewards[itemnr].item
            local label = Config.Rewards[itemnr].label
            if Config.framework == "redemrp" then
                local itemD = data.getItem(_source, item)
                itemD.AddItem(1)
            elseif Config.framework == "vorp" then
                VorpInv.addItem(_source, item, 1)
            elseif Config.framework == "qbr" then
                local User = exports['qbr-core']:GetPlayer(_source)
                User.Functions.AddItem(item, 1)
            end
            TriggerClientEvent("Notification:left_grave_robbery", _source, TEXTS.GraveRobbery, TEXTS.FoundItem.."\n+ "..label, TEXTURES.alert[1], TEXTURES.alert[2], 2000)
        else
            TriggerClientEvent("Notification:left_grave_robbery", _source, TEXTS.GraveRobbery, TEXTS.FoundNoItem, TEXTURES.alert[1], TEXTURES.alert[2], 2000)
        end
    end
end)
