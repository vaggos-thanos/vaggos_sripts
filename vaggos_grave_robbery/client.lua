            ...
            for i,v in pairs(Config.Graves) do 
                ...
                if dist < 1.0 then 
                    ...
                    if Citizen.InvokeNative(0xC92AC953F0A982AE,PromptKey) then
                        TriggerEvent("vaggos_grave_robbery:dig", i)
                        Citizen.Wait(2000)
                    end
                    if Citizen.InvokeNative(0xC92AC953F0A982AE,PromptKey2) then
                        TriggerEvent("vaggos_grave_robbery:pray", i)
                        Citizen.Wait(2000)
                    end
                end
            end
...
RegisterNetEvent("vaggos_grave_robbery:dig")
AddEventHandler("vaggos_grave_robbery:dig", function(id)
    if praying then 
    ...
    if digging then 
        ...
        TriggerServerEvent("vaggos_grave_robbery:check_shovel", id)
    end
end)
...
RegisterNetEvent("vaggos_grave_robbery:start_dig")
AddEventHandler("vaggos_grave_robbery:start_dig", function(id)
    if shovelObject then
        DeleteObject(shovelObject)
        SetEntityAsNoLongerNeeded(shovelObject)
        shovelObject = nil
    ...
    TriggerEvent("vaggos_grave_robbery:digging_timer",id)
    Citizen.Wait(200)
    RemoveAnimDict(anim[1])
    SetModelAsNoLongerNeeded(model)
end)

RegisterNetEvent("vaggos_grave_robbery:digging_timer")
AddEventHandler("vaggos_grave_robbery:digging_timer", function(id)
    ...
    if digging then 
        EndShovel()
        TriggerServerEvent("vaggos_grave_robbery:reward", id)
    end
end)

RegisterNetEvent("vaggos_grave_robbery:pray")
AddEventHandler("vaggos_grave_robbery:pray", function(id)
    if digging then 
        TriggerEvent("Notification:left_grave_robbery", TEXTS.GraveRobbery, TEXTS.CantDoThat, TEXTURES.alert[1], TEXTURES.alert[2], 2000)
        return
    end
...
AddEventHandler('Notification:left_grave_robbery', function(t1, t2, dict, txtr, timer)
    if Config.framework == "redemrp" then
        ...
        if txtr ~= nil then
            exports.vaggos_grave_robbery.LeftNot(0, tostring(t1), tostring(t2), tostring(dict), tostring(txtr), tonumber(timer))
        else
            local txtr = "tick"
            exports.vaggos_grave_robbery.LeftNot(0, tostring(t1), tostring(t2), tostring(dict), tostring(txtr), tonumber(timer))
        end
